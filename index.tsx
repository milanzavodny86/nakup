  ...
  return (
    <div className="flex flex-col h-screen max-w-md mx-auto bg-white overflow-hidden shadow-2xl relative">
      ...
      <div className="bg-blue-600 p-6 pt-12 pb-8 rounded-b-[2rem] shadow-lg shrink-0 z-10 text-white">
        <div className="flex justify-between items-start">
          <div>
            ...
            <p className="text-[10px] font-bold mt-2 opacity-80 tracking-widest uppercase">Offline Úložisko</p>
          </div>
          <div className="flex flex-col items-end gap-2">
            ...
            {items.some(i => i.status === 'bought') && (
              <button onClick={clearBought} className="text-[10px] font-bold underline opacity-70">
                VYMAZAŤ KÚPENÉ
              </button>
            )}
          </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3 no-scrollbar pb-32 bg-slate-50">
        {items.length === 0 ? (
          <div className="text-center py-32 opacity-30 italic font-bold">Zoznam je prázdny...</div>
        ) : (
          items.map(item => (
            <div 
              key={item.id}
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-[10px] font-black text-blue-500 uppercase mt-1">{item.category}</p>
                </div>
              </div>
              <button onClick={(e) => deleteItem(item.id, e)} className="text-slate-300 text-3xl px-2">&times;</button>
            </div>
          ))
        )}
      </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-50 flex items-end" onClick={() => setIsModalOpen(false)}>
          <div className="bg-white w-full p-8 rounded-t-[3rem] shadow-2xl space-y-5" onClick={e => e.stopPropagation()}>
            ...
            <input 
              autoFocus
              className="w-full bg-slate-50 p-5 rounded-2xl outline-none border border-slate-100 font-bold text-xl"
              placeholder="Názov..."
              value={newItem.name}
              ...
              {CATEGORIES.map(c => (
                <button
                  ...
                  className={`p-3 rounded-xl text-xs font-bold border ${
                    newItem.category === c ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-slate-600 border-slate-100'
                  }`}
                >
                  {c}
                </button>
            ...
            <button onClick={addItem} className="w-full bg-blue-600 text-white py-5 rounded-2xl font-black uppercase shadow-xl active:scale-95">
              Pridať do zoznamu
            </button>
          </div>
        </div>
...
// Spustenie cez ReactDOM.createRoot z modulu react-dom/client
